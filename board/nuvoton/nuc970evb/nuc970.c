...
int board_init(void)
{
    gpio_request(GPIO_PG3,"Buzzer");
    gpio_direction_output(GPIO_PG3,1);
    mdelay(100);
    gpio_direction_output(GPIO_PG3,0);

    gpio_request(GPIO_PB11,"5V_reg");
    gpio_direction_output(GPIO_PB11,1);

    gpio_request(GPIO_PE15,"USB_HUB Enable");
    gpio_direction_output(GPIO_PE15,1);

    gpio_request(GPIO_PF7,"USB ID Pull Down");
    gpio_direction_output(GPIO_PF7,0);

    gpio_request(GPIO_PB2,"BackLight");
    gpio_direction_output(GPIO_PB2,1);

    gpio_request(GPIO_PJ3,"2V8_reg");
    gpio_direction_output(GPIO_PJ3,1);

    /*For Testing*/
    gpio_request(GPIO_PB10,"3.8V");
    gpio_direction_output(GPIO_PB10,1);

    gpio_request(GPIO_PE9,"PRINTER MOTOR");
    gpio_direction_output(GPIO_PE9,1);

    gpio_request(GPIO_PH2,"PRINTER MOTOR");
    gpio_direction_output(GPIO_PH2,1);

    gpio_request(GPIO_PJ0,"GPRS ON");
    gpio_direction_output(GPIO_PJ0,1);

    gpio_request(GPIO_PB3,"RFID PWR ON");
    gpio_direction_output(GPIO_PB3,1);

    gpio_request(GPIO_PD13,"RFID IO EN");
    gpio_direction_output(GPIO_PD13,1);
    /*For Testing*/
